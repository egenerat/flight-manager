...
class AirportChecker(object):
    ...
    def __check_missing_planes(self):
    ...
    def __fix_missing_planes(self):
        ...
        if total_missing_planes > 0:
            ...
            for plane_class, missing_units in self.__check_missing_planes().iteritems():
                if missing_units > 0:
                    self.airport_buyer.buy_missing_planes(
                        plane_class, min(missing_units, total_missing_planes))
                    # TODO new_planes var should be set with the result of buying_missing_planes

    def __fix_missing_staff(self):
        ...
        flight_attendants_diff = staff_config.flight_attendants_nb - \
            current_staff.total_flight_attendants
        if flight_attendants_diff > 0:
            self.injected_staff_buyer.hire_flight_attendants(
                flight_attendants_diff)
    ...
    def __fix_missing_resources(self):
