...
from app.common.constants import SUPERSONICS_MISSION_PAGE, TIME_DIFFERENCE
...
def parse_duration_before_departure(html_mission):
    ...
    '<td class ="Mpossibles2"> Date d&eacute;part : 10/06/2016 \xc3\xa0 11:26 </td>'
...
def parse_one_mission(mission_html, country_nb):
    if not string_contains(u'Mission impossible : la distance entre cette ville et votre aéroport est trop courte',mission_html) and not string_contains(u"vous n'avez pas d'avion correspondant &agrave; cette mission", mission_html):
        ...
        a_mission = {
            ...
            u'travellers_nb': get_int_from_regex(u'Nombre de (?:passagers|marchandises) : <strong>(\d+)</strong>',mission_html),
            ...
            u'pilotes_nb': get_int_from_regex(u'équipage requis : <strong>(\d+)</strong> pilote',mission_html),
            ...
            u'km_nb': get_int_from_regex(u'<td class="Mpossibles2">Distance : <strong>(\d+)</strong> km</td>',mission_html)
        }
        return a_mission
...
def substract(missions_list, ongoing_missions_id):
    result = []
